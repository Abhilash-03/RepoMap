...
function DocsHeader({ isSidebarOpen, onToggleSidebar }: DocsHeaderProps) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
      <div className="flex items-center justify-between h-16 px-4 sm:px-6">
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          <Link to="/" className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 hidden sm:block">Documentation</p>
            </div>
          </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <Link
            to="/"
            className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 hover:text-violet-600 dark:hover:text-violet-400 transition-colors p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
          >
